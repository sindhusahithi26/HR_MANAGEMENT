...
public class Add {
    public static void addEmployee (String fname,String lname,String role,String gender ,String dob,String dj,String mail,String phno,String city,String zip,String Sal,String qualify,String depart,String Depid,String Train ) {
        ...
        if (!Validator.name(fname+lname)) {
            ...
            String ins = String.format("Insert into employee values ('%s','%s','%s','%s','%s','%s','%s','%s',%d,'%s',%d,'%s','%s','%s','%s','%s')",
            		"E0"+Integer.toString(nempid),fname.toLowerCase().trim(),lname.toLowerCase().trim(),gender,dob,mail.toLowerCase().trim(),phno,city.trim(),Integer.parseInt(zip),dj,Integer.parseInt(Sal),role.trim(),qualify.trim(),depart,Depid,Train.trim());  
            Statement stt;
            try {
